...
// STATIC FALLBACK DATA (For Weather only now)
const LOCALITY_DATA = {
  "BTM Layout": {
    weather: {
      temp: "29°C",
...
const Home = () => {
  ...
  // LIVE HOOKS
  const { places: dynamicPlaces, loading: loadingPlaces } = useNearbyPlaces(
    coords.lat,
    coords.lng,
    selectedCategory,
  );
  // UPDATED: Now destructuring newsList instead of a single localNews object
  const { newsList, loading: loadingNews } = useLocalityNews(localityName);

  const { weather: localWeather, loading: loadingWeather } = useLocalityWeather(
    coords.lat,
    coords.lng,
  );
  ...
  // Calculate the invisible fence for the map (roughly 3km around the center)
  const bounds = L.latLngBounds(
    [coords.lat - 0.03, coords.lng - 0.03],
    [coords.lat + 0.03, coords.lng + 0.03],
  );
  ...
  return (
    ...
    <header className={styles.header}>
        ...
        <p className={styles.subtitle}>
          {localityName
            ? `Dashboard for ${localityName}`
            : "Select your locality to see updates"}
        </p>
      </header>
      ...
      {localityName && (
        <div className={styles.dashboard}>
            ...
            <div className={styles.envCard}>
              ...
              <div className={styles.envData}>
                <span className={styles.envValue}>
                  {loadingWeather ? "..." : localWeather?.temp || "N/A"}
                </span>
                <span className={styles.envLabel}>
                  {loadingWeather
                    ? "Loading"
                    : localWeather?.condition || "N/A"}
                </span>
              </div>
            </div>
            <div className={styles.envCard}>
              ...
              <div className={styles.envData}>
                <span className={styles.envValue}>
                  {loadingWeather ? "..." : `${localWeather?.aqi || "N/A"} AQI`}
                </span>
                <span className={styles.envLabel}>
                  {loadingWeather ? "Loading" : localWeather?.quality || "N/A"}
                </span>
              </div>
            </div>
          </div>
          ...
          {/* QUICK SEARCH & DYNAMIC PLACES LIST */}
            ...
            <div className={styles.quickNavGrid}>
              {quickLinks.map((link) => (
                <button
                  ...
                  onClick={() =>
                    setSelectedCategory(
                      selectedCategory === link.name ? null : link.name,
                    )
                  }
                >
            ...
            {selectedCategory && (
              <div className={styles.placesList}>
                {loadingPlaces && (
                  ...
                  >
                    Scanning satellite data for nearby {selectedCategory}...
                  </p>
                )}
                ...
                {!loadingPlaces && dynamicPlaces.length === 0 && (
                  ...
                  >
                    No {selectedCategory} found within 3km of this center.
                  </p>
                )}
                      ...
                      <div className={styles.placeCardActions}>
                        {place.phone !== "Not listed" && (
                          <a
                            ...
                            className={styles.placeActionBtn}
                        ...
                        <a
                          ...
                          className={`${styles.placeActionBtn} ${styles.navigateBtn}`}
            ...
            <div className={styles.newsContainer}>
              {loadingNews ? (
                ...
                <div className={styles.newsCard}>
                  ...
                  >
                    No updates available for {localityName} today.
                  </p>
                </div>
              )}
            </div>
